...
func (m *ListSortAl14) Less(i, j int) bool {
	d1, _ := time.Parse(constant.DATE_DASH_FORMAT, m.List[i].FinalModifyDate)
	d2, _ := time.Parse(constant.DATE_DASH_FORMAT, m.List[j].FinalModifyDate)
	if d1.Equal(d2) {
		t1, _ := time.Parse(constant.TIME_COLON_FORMAT, m.List[i].FinalModifyTime)
		t2, _ := time.Parse(constant.TIME_COLON_FORMAT, m.List[j].FinalModifyTime)
		b := t1.After(t2)
		return b
	}
	a := d1.After(d2)
