...
type AL000001O struct {
	...
	TAcctFee                    []TAcctFee                   `json:"tAcctFee"`
}
...
type TLoanAcctgAcct struct {
	...
	TurnImpairmentDate      string          `json:"turnImpairmentDate"`
}
...
type TLoanInterestAccrual struct {
	...
	FinalModifyOrgz        string          `json:"finalModifyOrgz"`
	FinalModifyUser        string          `json:"finalModifyUser"`
	PenaltyInterestAccrual decimal.Decimal `json:"penaltyInterestAccrual"`
	PenaltyInterestCalc    decimal.Decimal `json:"penaltyInterestCalc"`
	PenaltyInterestPaid    decimal.Decimal `json:"penaltyInterestPaid"`
}
...
type TLoanInterestAccrualPeriod struct {
	...
	FinalModifyOrgz       string          `json:"finalModifyOrgz"`
	FinalModifyUser       string          `json:"finalModifyUser"`
	InterestOverdue       decimal.Decimal `json:"interestOverdue"`
}
...
type AmortizationPeriod struct {
	Period                 int             `json:"period"  xorm:"not null  int(6)"`
	AmountType             string          `json:"amount_type" xorm:"not null  varchar(4)"`
	CurrentStatus          string          `json:"current_status" xorm:"not null  varchar(1)"`
	CurrentAccountStatus   string          `json:"current_account_status"  xorm:"not null  comment('0-正常;1-逾期;2-减值;3-核销;4-结清') varchar(1)"`
	CurrentPeriodBeginDate string          `json:"current_period_begin_date"  xorm:"not null  date"`
	CurrentPeriodEndDate   string          `json:"current_period_end_date"  xorm:"not null  date"`
	CurrentRepayDate       string          `json:"current_repay_date"  xorm:"not null  date"`
	Currency               string          `json:"currency"  xorm:"not null  comment('CM0001') varchar(3)"`
	PlanRepayPrincipal     decimal.Decimal `json:"plan_repay_principal"  xorm:"not null  decimal(18,2)"`
	ActualRepayDate        string          `json:"actual_repay_date"  xorm:"date"`
	ActualRepayPrincipal   decimal.Decimal `json:"actual_repay_principal"  xorm:"decimal(18,2)"`
	CurrentUnpaidPrincipal decimal.Decimal `json:"current_unpaid_principal"  xorm:"not null  decimal(18,2)"`
	LastActiveDate         string          `json:"last_active_date"  xorm:"not null  date"`
	PlanRepayInterest      decimal.Decimal `json:"plan_repay_interest"  xorm:"not null  decimal(18,2)"`
	ActualRepayInterest    decimal.Decimal `json:"actual_repay_interest"  xorm:"not null  decimal(18,2)"`
	CloseDate              string          `json:"close_date"  xorm:"date"`
